...
module "athena_crawler_role" {
  source = "git@ssh.dev.azure.com:v3/SempraUtilities/SempraUtilities/terraform-aws-seu-iam-role?ref=release"
  company_code      = local.company_code
  application_code  = local.application_code
  environment_code  = local.environment_code
  region_code       = local.region_code
  application_use   = "glue-crawler-test-${local.application_use}"
  description       = "Test IAM role for Athena"
  service_resources = ["glue.amazonaws.com", "lakeformation.amazonaws.com"]
...
resource "aws_iam_policy" "policy" {
  name        = "policy-test-${local.application_use}"
...
resource "aws_iam_role_policy_attachment" "attach-custom" {
...
resource "aws_iam_role_policy_attachment" "attach-managed-athena" {
  role       = module.athena_crawler_role.id
...
module "athena" {
  ...
  athena_db_name         = "athena_db_${local.application_use}"
  ...
  named_query_name          = "iac_test_query_${local.application_use}"
...
module "glue-crawler" {
  source = "git@ssh.dev.azure.com:v3/SempraUtilities/SempraUtilities/terraform-aws-seu-glue-crawler?ref=release"
  company_code     = local.company_code
  application_code = local.application_code
  environment_code = local.environment_code
  region_code      = local.region_code
  application_use  = "s3-${local.application_use}"

  iam_role_arn  = module.athena_crawler_role.arn
  iam_role_name = module.athena_crawler_role.id

  glue_database_map = {
    test_data_db = { // Maybe need to update
      // job specific name here gets appended to standardized name
      name = "db-${local.application_use}"
      optional_arguments = {
        description = "This is database 1"
      }
    }
  }
  ...
  glue_crawler_map = {
    crawler_s3 = {
      name          = "crawler"
      database_name = "db-${local.application_use}"
      s3_targets = {
        s3_target1 = {
